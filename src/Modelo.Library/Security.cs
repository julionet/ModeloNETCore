...
namespace Modelo.Library
{
    public class Security
    {
        ...
        public static string GetHashAutenticacaoWebapi(string token)
        {
            string _passwordKey = "_GxYhJ@159753_";
            return Convert.ToBase64String(Encoding.GetEncoding("ISO-8859-1").GetBytes($"{_passwordKey}|{token}"));
        }
        ...
        public static string CreatePasswordWeb()
        {
            string senha = (char)(new Random().Next(65, 90)) + new Random().Next(9999).ToString("0000");
            Thread.Sleep(300);
            return  senha + (char)(new Random().Next(65, 90));
        }
    }
}
